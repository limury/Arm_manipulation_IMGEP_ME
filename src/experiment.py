...
class Experiment(object):
    ...
    def execute_perceive(self, m):
        ...
        s = self.environment.update(m, gui=False) # execute this command and observe the corresponding sensory effect
    ...
    def dump(self, filename):
        self.dat = np.reshape(self.data, (-1, 175))
        output = [None]*(np.shape(self.dat)[0])
        for x in range(len(self.dat)):
            tw = self.dat[x] / 4
            tw = tw + 0.5
            tw = tw.tolist()
            # [ hand_x, hand_y, stick1_x, stick1_y, stick2_x, stick2_y, magnet1_x, magnet1_y, scratch1_x, scratch1_y]
            output[x] = [tw[-31], tw[-30], tw[-28], tw[-27], tw[-26], tw[-25], tw[-24], tw[-23], tw[-18], tw[-17]] 
        
        #print(output)
        with open(self.log_dir + "/" + filename + ".dat", 'w') as f:
            wr = csv.writer(f)
            wr.writerows(output)
        
    def run(self, iterations=100000, profile=False, print_logs=False, dump_period=100000, presets=""):
        self.curr_i = dump_period 
        if profile:
            cp = cProfile.Profile()
            cp.enable()
        ...
        while self.i < iterations:
            ...
            if (self.i >= self.curr_i):
                gen = '0'*(8-len(str(self.curr_i)))
                gen+= str(self.curr_i)
                self.dump(filename=presets + gen) 
                self.curr_i += dump_period

        gen = '0'*(8-len(str(iterations)))
        gen+= str(iterations)
        self.dump(filename=presets + gen) 
